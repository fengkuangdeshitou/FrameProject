...
+(void)hs_updateWithAPPID:(NSString *)appId withBundleId:(NSString *)bundelId block:(UpdateBlock)block{
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"appInfo: %@", appInfoDic[@"results"][0][@"releaseNotes"]);
            ...
            NSString *currentVersionTemp = [NSString stringWithFormat:@"%@", currentVersion];
            ...
            if([currentVersion floatValue] < [appStoreVersion floatValue])
            {
                ...
                block(currentVersionTemp,appInfoDic[@"results"][0][@"version"],appInfoDic[@"results"][0][@"trackViewUrl"],YES, appInfoDic[@"results"][0][@"releaseNotes"]);
            }else{
                NSLog(@"【4】判断结果：当前版本号%@ > 商店版本号%@ 不需要更新\n========我是分割线========",[[NSBundle mainBundle] infoDictionary][@"CFBundleShortVersionString"],appInfoDic[@"results"][0][@"version"]);
                block(currentVersionTemp,appInfoDic[@"results"][0][@"version"],appInfoDic[@"results"][0][@"trackViewUrl"],NO, @"");
            }
        });
    }];
    [task resume];
