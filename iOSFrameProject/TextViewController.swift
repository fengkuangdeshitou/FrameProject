...
class TextViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let _ = textView.es.addPullToRefresh {
            [weak self] in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                ...
                weakSelf.textView.attributedText = NSAttributedString.init(string: weakSelf.text1, attributes: [NSAttributedString.Key.paragraphStyle : style, NSAttributedString.Key.font: UIFont.init(name: "ChalkboardSE-Bold", size: 16.0)!, NSAttributedString.Key.foregroundColor: UIColor.init(white: 0.3, alpha: 1.0)])
                weakSelf.textView.es.stopPullToRefresh(ignoreDate: true, ignoreFooter: true)
            }
        }
        
        let _ = textView.es.addInfiniteScrolling {
            [weak self] in
            guard let weakSelf = self else {
                return
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                ...
                if weakSelf.num >= 7 {
                    weakSelf.textView.es.noticeNoMoreData()
                } else {
                    let style = NSMutableParagraphStyle.init()
                    style.lineSpacing = 0.0
                    style.firstLineHeadIndent = 10.0
                    style.alignment = .justified
//                    weakSelf.textView.attributedText = NSAttributedString.init(attributedString: [NSAttributedString.Key.foregroundColor:UIColor.white])
