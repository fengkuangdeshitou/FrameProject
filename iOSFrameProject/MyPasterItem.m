...
-(void)initialView
{
    ...
    UITapGestureRecognizer *rotateTapGes = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(editPasterTapGes:)];
 ...
 *  @param ges 编辑手势
 */
-(void)editPasterTapGes:(UITapGestureRecognizer *)ges
{
    if ([self.customDelegate respondsToSelector:@selector(myPasterItem:pasterEdit:)]) {
        [self.customDelegate myPasterItem:self pasterEdit:self.paster];
    }
}
...
-(void)setIsSelect:(BOOL)isSelect
{
    ...
    if (_isSelect == YES)
    {
        ...
        _sizeView.hidden = NO;
        if (self.paster.indexTag == 0 || self.paster.indexTag == 1) {
