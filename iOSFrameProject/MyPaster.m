...
@interface MyPaster () <MyPasterItemDelegate>
...
-(void)addImagePaster:(ImagePaster *)paster
{
    ...
    if (_contentImageView.image != nil && paster.image != nil)
    {
        ...
        tpPoint = CGPointMake((self.frame.size.width - imageSize.width)/2, (self.frame.size.height - imageSize.height)/2);
...
-(void)addTextPaster:(TextPaster *)paster
{
    if (_contentImageView.image != nil)
    {
        ...
        tpPoint = CGPointMake((self.frame.size.width - paster.size.width)/2, (self.frame.size.height - paster.size.height)/2);
...
-(void)layoutSubviews
{
    ...
    if (_contentImageView.image != nil)  // image 异常
    {
    ...
    @try {
        _contentImageView.center = CGPointMake((NSInteger)self.bounds.size.width/2.0, (NSInteger)self.bounds.size.height/2.0);
    } @catch (NSException *exception) {
        NSLog(@"_contentImageView.center 设置出错");
    }
...
// MARK: - MyPasterItemDelegate
// MARK: pasterEdit
- (void)myPasterItem:(MyPasterItem *)myPasterItem pasterEdit:(Paster *)paster {
    if ([self.delegate respondsToSelector:@selector(myPaster:pasterEdit:)]) {
        [self.delegate myPaster:self pasterEdit:paster];
    }
}
