...
- (instancetype)initWithFrame:(CGRect)frame norImageArr:(NSArray *)norImageArr SelImageArr:(NSArray *)selImageArr TitleArr:(NSArray *)titleArr Config:(JMConfig *)config{
    if (self = [super initWithFrame:frame]) {        
        for (int i = 0; i < titleArr.count; i++) {
            ...
            tbBtn.title.textColor = [[JMConfig shareConfig] norTitleColor];
        ...
        //背景颜色处理
        self.backgroundColor = [[JMConfig shareConfig] tabBarBackground];
...
- (void)tapClick:(UITapGestureRecognizer *)tap {
    // willShowSeletView
    BOOL isNext = true;
    if ([self.myDelegate respondsToSelector:@selector(willShowSelectViewWith:didSelectIndex:)]) {
        isNext = [self.myDelegate willShowSelectViewWith:self didSelectIndex:tap.view.tag];
    }
...
- (void)setUpSelectedIndex:(NSInteger)selectedIndex {
    ...
    for (int i = 0; i < self.saveTabBarArrM.count; i++) {
        ...
        if (i == selectedIndex) {
            tbBtn.title.textColor = [[JMConfig shareConfig] selTitleColor];
            tbBtn.imageView.image = [UIImage imageNamed:self.selImageArrM[i]];
            
            JMConfigTabBarAnimType type = [[JMConfig shareConfig] tabBarAnimType];
...
- (void)topLineIsClearColor:(BOOL)isClearColor {
    ...
    if (!isClearColor) {
        color = [[JMConfig shareConfig] tabBarTopLineColor];
    }
    
    CGRect rect = CGRectMake(0, 0, self.width, 0.6);
