...
class JQEmitterButton: UIButton {
    ...
    // MARK: - 显示泡泡动画
    @objc public func showPaoPaoAnimation() {
        ...
        blingImageView.center = CGPoint(x: bounds.width-10, y: 0)
        ...
        UIView.animate(withDuration: upDuration, delay: 0, options: .curveLinear, animations: {
            ...
            UIView.animate(withDuration: upDuration, delay: 0, options: .curveLinear, animations: {
                blingImageView.transform = CGAffineTransform(scaleX: 3, y: 3)
    ...
    fileprivate func blingBling(_ view: UIView) {
        ...
        for i in 0..<count {
            ...
            path.addLine(to: CGPoint(x: Double(view.bounds.width/2)+20*cos(2*Double.pi*Double(i)/Double(count)), y: Double(view.bounds.width/2)+20*sin(2*Double.pi*Double(i)/Double(count))))
    ...
    fileprivate func bubbleAnimation(start: CGPoint, isAuto: Bool) {
        ...
        let bubble = UIImageView(image: UIImage(named: "detail_support_ani"))
        bubble.bounds = CGRect(x: 0, y: 0, width: 40, height: 40)
...
extension JQEmitterButton {
    ...
    func emit(count: uint) {
        ...
        /*
         需要Timer
         */
        Timer.scheduledTimer(timeInterval: TimeInterval(interval), target: self, selector: #selector(timerRunLoopFunc(timer:)), userInfo: nil, repeats: true)
    }
    
    // MARK: timer runloop
    @objc func timerRunLoopFunc(timer: Timer) {
